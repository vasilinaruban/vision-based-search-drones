...
Процесс обучения
    ...
    Подготовка данных:
        ...
        Создается файл data.yaml с путями к данным и конфигурацией классов

    Эксперименты с параметрами:
    В скрипте проведены эксперименты с различными параметрами:

        Размер изображения (imgsz): 480, 640, 800 пикселей

            Выводы: увеличение размера повышает precision, но снижает recall

            Лучший mAP75 достигнут при imgsz=480

        Версии моделей: n, s, m

            Выводы: модель YOLOv8s показала лучшие результаты
        ...
        Гиперпараметры:
            ...
            Аугментации (perspective, translate)

            Dropout

            Коэффициент box loss

    Лучшие результаты:

        mAP50-95: 0.728 (optimizer_SGD)

        mAP50: 0.949 (freeze_10, translate_03, multi_scale)

        mAP75: 0.815 (optimizer_SGD)

        Precision: 0.981 (freeze_10)

        Recall: 0.883 (perspective_0001, box)
